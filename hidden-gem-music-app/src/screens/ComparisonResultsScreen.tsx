...
export type Props = {
  ...
  onOpenHiddenGemsForCountry: (
    countryId: string,
    selection?: { songTitle?: string; artist?: string; previewIndex?: number; deezerTrackId?: number }
  ) => void;
};
...
function StatSquare({
  ...
  return (
    <CountryPageSection style={[styles.statSquare, style]} contentStyle={styles.statSquareContent}>
      ...
      {typeof value === "string" ? <Text style={styles.statSquareValue}>{value}</Text> : value}
          ...
          {songs.length === 0 && !isInitialLoading ? (
            <View style={styles.sectionFallbackBody}>
              <Text style={styles.sectionFallbackText}>
                Unfortunately at this time, there is not enough data needed for this country in this year to display this section. Try another year and/or country.
              </Text>
            </View>
          ) : null}
    ...
    <CountryPageSection style={styles.hiddenSongsCarouselSection} contentStyle={styles.hiddenSongsCarouselSectionContent}>
      <View style={[styles.hiddenSongsCarouselHeader, isCompact ? styles.hiddenSongsCarouselHeaderCompact : null]}>
        <View style={[styles.hiddenSongsCarouselHeaderLeft, isCompact ? styles.hiddenSongsCarouselHeaderLeftCompact : null]}>
          ...
          <Text style={[styles.hiddenSongsCarouselHelper, isCompact ? styles.hiddenSongsCarouselHelperCompact : null]}>
            {isLoading ? loadingText : sectionAvailable ? "Click a song to listen to a 30 second preview on the Hidden Gems page." : ""}
          </Text>
        </View>
        <Pressable
          onPress={() => {
      ...
      {!isLoading && !sectionAvailable ? (
        ...
        <CarouselArrowButton
          direction="left"
          onPress={goPrevious}
          style={isCompact ? styles.hiddenSongsCarouselArrowLoweredNative : styles.hiddenSongsCarouselArrowLowered}
        />
...
function ComparisonCountryPane({
  ...
  onChangeCountry: (countryId: string) => void;
  onOpenCountry: (countryId: string) => void;
  onOpenHiddenGemsForCountry: (countryId: string, selection?: { songTitle?: string; artist?: string; previewIndex?: number; deezerTrackId?: number }) => void;
}) {
  ...
  const countryOptions = useMemo(
    ...
    [availableCountries, country.id, selectedYear, unavailableCountryId]
  );
  const openForCountry = (selection?: { songTitle?: string; artist?: string; previewIndex?: number; deezerTrackId?: number }) =>
  ...
  }, [country.code, country.name, isActive, selectedYear]);
  ...
  const loadMoreUniqueSongs = () => {
    ...
    loadCountrySongsPage(country.code, selectedYear, "unique", nextPage, songPageSize, controller.signal)
      ...
      .catch((error) => {
        if (latestRequestRef.current === requestId && !(error instanceof Error && error.name === "AbortError")) {
            ...
            value={
              areProfileStatsLoading ? (
                ...
                <Text style={styles.statSquareValue}>
                  ...
                  {isMobileStack ? null : <Text style={styles.statSquarePercentSymbol}>%</Text>}
                </Text>
              )
            }
            note="% of this view"
      ...
      <View style={styles.screen}>
        {isMobileStack ? (
          <View style={styles.mobileComparisonHeader}>
            <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.62} style={styles.mobileComparisonHeaderText}>
